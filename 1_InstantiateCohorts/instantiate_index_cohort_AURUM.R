...
start.date1 <- as.Date("2021-01-04") # Start date of the study 1 (first vaccination included)
end.date1 <- as.Date("2021-01-27") # End date of the study 1 (last vaccination included)
start.date2 <- as.Date("2021-01-28") # Start date of the study 2 (first vaccination included)
end.date2 <- as.Date("2021-02-28") # End date of the study 2 (last vaccination included)
start.date3 <- as.Date("2021-03-01") # Start date of the study 3 (first vaccination included)
end.date3 <- as.Date("2021-04-13") # End date of the study 3 (last vaccination included)
start.date4 <- as.Date("2021-04-14") # Start date of the study 4 (first vaccination included)
end.date4 <- as.Date("2021-07-31") # End date of the study 4 (last vaccination included)
...
shielding_id <- cohortSet(cdm[[databaseSpecificCohortName]]) %>%
  filter(cohort_name == "shielded") %>%
  pull("cohort_definition_id")

at_risk_id <- cohortSet(cdm[[databaseSpecificCohortName]]) %>%
  filter(cohort_name == "at_risk") %>%
  pull("cohort_definition_id")

high_risk_id <- cohortSet(cdm[[databaseSpecificCohortName]]) %>%
  filter(cohort_name == "high_risk_with_no_low_or_moderate_later") %>%
  pull("cohort_definition_id")
...
vaccinated <- cdm[[vaccinatedCohortName]] %>%
  dplyr::inner_join(
    cohortSet(cdm[[vaccinatedCohortName]]) %>%
...
# shielding cohort
shielding <- cdm[[databaseSpecificCohortName]] %>%
  dplyr::filter(.data$cohort_definition_id == .env$shielding_id) %>%
  dplyr::select(
    "person_id" = "subject_id",
    "shielding_date" = "cohort_start_date"
  ) %>%
  dplyr::compute()

# high risk
high_risk <- cdm[[databaseSpecificCohortName]] %>%
  dplyr::filter(.data$cohort_definition_id == .env$high_risk_id) %>%
  dplyr::select("person_id" = "subject_id") %>%
  dplyr::compute()

# at risk
at_risk <- cdm[[databaseSpecificCohortName]] %>%
  dplyr::filter(.data$cohort_definition_id == .env$at_risk_id) %>%
  dplyr::select(
    "person_id" = "subject_id",
...
### with an England associated care site id
...
attrition <- rbind(
  ...
  dplyr::tibble(
    number_observations = number_observations,
    reason = "Do not have england location",
  ...
  dplyr::left_join(
    individuals %>%
      ...
      dplyr::inner_join(
        dplyr::union(
          shielding %>%
            dplyr::filter(.data$shielding_date <= .env$start.date2) %>%
            dplyr::select("person_id"),
          high_risk
        ) %>%
          dplyr::distinct(),
...
individuals <- individuals %>%
  dplyr::left_join(
    individuals %>%
      ...
      dplyr::inner_join(
        at_risk %>%
          dplyr::filter(.data$at_risk_date <= .env$start.date2) %>%
          dplyr::select("person_id") %>%
          dplyr::distinct(),
        by = "person_id"
      ) %>%
...
vaccinatedCohortSet <- cohortSet(cdm[[vaccinatedCohortName]]) %>% 
  collect() %>%
...
create_study_cohort <- function(study_cohort, s, start.date, end.date) {
  ...
  attr(study_cohort_all, "cohortSet") <- dplyr::tibble(
    cohort_definition_id = c(
      any_vaccine, astrazeneca, pfizer, un_any_vaccine, un_astrazeneca, 
      un_pfizer
    ),
    cohort_name = c(
